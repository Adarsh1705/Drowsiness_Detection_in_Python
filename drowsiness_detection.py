...
CONSECUTIVE_FRAMES_EYES = 20
# Declare another costant to hold the consecutive number of frames to consider for a yawn
CONSECUTIVE_FRAMES_MOUTH = 10
...
while True:
    ...
    for (i, rect) in enumerate(rects):
        ...
        if EAR < EAR_THRESHOLD:
            ...
            if FRAME_COUNT_EYES >= CONSECUTIVE_FRAMES_EYES:
                count_sleep += 1
                # Add the frame to the dataset ar a proof of drowsy driving
                cv2.imwrite("dataset/frame_sleep%d.jpg" % count_sleep, frame)
                # playsound('sound files/alarm.mp3')
                winsound.Beep(32767, 250)
                cv2.putText(frame, "DROWSINESS ALERT!", (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        else:
            if FRAME_COUNT_EYES >= CONSECUTIVE_FRAMES_EYES:
                # playsound('sound files/warning.mp3')
                winsound.Beep(32767, 250)
                cv2.putText(frame, "DROWSINESS WARNING!", (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (51, 153, 255), 2)
            FRAME_COUNT_EYES = 0
        ...
        if MAR > MAR_THRESHOLD:
            ...
            if FRAME_COUNT_MOUTH >= CONSECUTIVE_FRAMES_MOUTH:
                count_yawn += 1
